...
class QueensConstraint(BinaryConstraint):
	def isSatisfied(self, value1, value2):
		# two adjacent queens in the same row
		if value1 == value2:
			return False
		# two adjacent queens in same diagonal
		if abs(ord(self.var1) - ord(self.var2)) == abs(int(value1) - int(value2)):
			return False
		return True
...
def consistent(assignment, csp, var, value):
 	"""Question 1"""
	# test_cases\q1\consistent2consistent2.test
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			...
			if assignment.isAssigned(other):
				if not constraint.isSatisfied(assignment.assignedValues[other], value):
					return False
	# print var # D
	# print value # R
	# print csp.binaryConstraints[1] # NotEqualConstraint (A, D)
	# print csp.binaryConstraints[1].affects(var) # True
	# print csp.binaryConstraints[1].otherVariable('D') # A
	# print assignment.assignedValues['A'] # B
	# print csp.binaryConstraints[1].isSatisfied(assignment.assignedValues['A'], 'B') # False
	# print assignment.isAssigned('A') # True
	return True
...
"""
	...
	# MRV: choosing the variable with the fewest "legal" values first.
	# Degree heuristic is used to break ties when variables have the same number of options.
	# python27 autograder.py -t test_cases\q2\minimumremainingvalues2mrvonly.test
def minimumRemainingValuesHeuristic(assignment, csp):
	nextVar = None
	domains = assignment.varDomains
	"""Question 2"""
	minSize = len(csp.varDomains.items()[0][1]) # number of initial values that each variable can take, 4
	minVar = csp.varDomains.items()[0][0] # the initial variable, A

	# find the number of constraints on other unassigned variables
	def findNumberOfConstraints(var, csp):
		degree = 0
		for constraint in csp.binaryConstraints:
			if constraint.affects(var):
				other = constraint.otherVariable(var)
	...
	for var, value in domains.items():
		if not assignment.isAssigned(var):
			if len(value) < minSize:
				minVar = var
				minSize = len(value)
			# if the number of options are tied, use findNumberOfDegree (degree heuristic) to break tie.
			if len(value) == minSize and findNumberOfConstraints(var, csp) >= findNumberOfConstraints(minVar, csp):
				minVar = var
				minSize = len(value)
	nextVar = minVar
	return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3"""
	def countValidValues(other, val):
		if val in assignment.varDomains[other]:
			return 1
		return 0
	...
	for val in values:
		...
		for constraint in csp.binaryConstraints:
			if constraint.affects(var):
				other = constraint.otherVariable(var)
				num += countValidValues(other, val)
		pairs.append((val, num))
	pairs = sorted(pairs, key = lambda tup: tup[1])

	for pair in pairs:
		orderedValues.append(pair[0])
...
def forwardChecking(assignment, csp, var, value):
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			other = constraint.otherVariable(var)
			if not assignment.isAssigned(other):
				# if the connected variables have the same values in their domain
				if value in domains[other]:
					domains[other].remove(value) # no need to reassign to domains[other], e.g domains[other] = domains[other].remove(value)
					inferences.add((other, value))
					if len(domains[other]) == 0:
						# if the algorithm reveals an inconsistency, any inferences made should be reversed before ending the fuction.
...
def revise(assignment, csp, var1, var2, constraint):
	...
	for v2 in assignment.varDomains[var2]:
		satisfied = False
		# if not all(constraint.isSatisfied(v1, v2) for v1 in assignment.varDomains[var1]):
		# 	inferences.add((var2, v2))

		# 1. record the inference variable and value first, and remove them from domain later
	...
	# 2.1 (Check) if variable in inferences is more than those in domains,
	# this will cause the domain size of var2 less than 0, which is inconsistent, return null
	if len(inferences) >= len(assignment.varDomains[var2]):
		return None
	else:
		# 2.2 remove all the inference variable from domains
		for inference in inferences:
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	while len(queue) != 0:
		...
		newInferences = revise(assignment, csp, xi, xj, cons)
		# if newInferences is None, then reverse the previous removal
		if newInferences == None:
			for inference in inferences:
				assignment.varDomains[inference[0]].add(inference[1])
			return None # inconsistent

		# newInferences is not empty
		if len(newInferences) != 0:
			# if len(assignment.varDomains[xi]) == 0:
			# 	return None
			for constraint in csp.binaryConstraints:
				if constraint.affects(xj): # xj here
					queue.append((xj, constraint.otherVariable(xj), constraint))
			inferences = set(inferences.union(newInferences))
	return inferences
...
def AC3(assignment, csp):
	...
	while len(queue) != 0:
		...
		# if newInferences is None, no need to reverse the previous removal
		# because if the assignment fails at any point then there is no prior state to back up to,
		# this means that there is no solution to the CSP
		if newInferences == None:
			return None
