...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # find a list of mostLikelyGhostPositions (alive ghosts)
        mostLikelyGhostPositions = []
        for livingGhostPositionDistribution in livingGhostPositionDistributions:
            mostLikelyGhostPositions.append(livingGhostPositionDistribution.argMax())
            # print mostLikelyGhostPosition

        # find the closest ghost position among all alive ghosts position
        closestGhost = mostLikelyGhostPositions[0]
        closestDistance = self.distancer.getDistance(closestGhost, pacmanPosition)
        for ghost in mostLikelyGhostPositions[1 : ]:
            distance = self.distancer.getDistance(ghost, pacmanPosition)
            if distance < closestDistance:
                closestGhost = ghost
    
        # find the best action that can shorten the greatest distance between pacman and ghost
