...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                # Bayesian Rule:
                # P(trueDistance | noisyDistance) = P(noisyDistance | trueDistance) * P(trueDistance) / P(noisyDistance)

                # The emission model would give you the likelihood. Specifially, it is the likelihood of recieving a noisyDistance
                # with a specific trueDistance. It's like saying, "I'm going to assume that pacman is at (a, b), which is x units
                # away from me. What is likelihood (or probability) that I receive a noisyDistance reading that I actually observed as evidence?"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # suppose ghost can be at any legal position
        for oldPos in self.legalPositions:
            ...
            # print newPosDist
            for newPos, prob in newPosDist.items():
                # P( ghost is at position p at time t + 1 | ghost is at position oldPos at time t ) * P( ghost is at position oldPos at time t )
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "Begin with a uniform distribution over ghost positions."
        self.particleBeliefs = []
        currentParticles = self.numParticles

        while currentParticles > 0:
            for p in self.legalPositions:
                self.particleBeliefs.append(p)
    ...
    def observe(self, observation, gameState):
        ...
        # that the ghost appears in its prison cell, self.getJailPosition()
        if noisyDistance == None:
            self.particleBeliefs = [self.getJailPosition()] * self.numParticles
        else:
            for particle in self.particleBeliefs:
                trueDistance = util.manhattanDistance(particle, pacmanPosition)
                ## 1. Weight ##
                # store the weight sum for each position and sample based on the current weight
                if emissionModel[trueDistance] > 0: weight[particle] += emissionModel[trueDistance]
            # when all particles receive 0 weight, they should be recreated from the 
            # prior distribution by calling initializeUniformly.
            # 300 particles
            if weight.totalCount() == 0:
                ...
                ## 2. Resample ##
                for particle in self.particleBeliefs:
                    newParticleBeliefs.append(util.sample(weight))
                self.particleBeliefs = newParticleBeliefs
    ...
    def elapseTime(self, gameState):
        ...
        newParticleBeliefs = []
        for particle in self.particleBeliefs:
            # newPosDist is a util.Counter object, where for each position p in self.particleBeliefs
            # newPosDist[p] = Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            ## 3. Propagate ##
            newParticleBeliefs.append(util.sample(newPosDist)) 
        self.particleBeliefs = newParticleBeliefs
    ...
    def getBeliefDistribution(self):
        ...
        """
        # self.particleBeliefs : [(x1, y1),(x2, y2), ... ]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        baseBeliefs = [x for x in itertools.product(self.legalPositions, repeat=self.numGhosts)]
        random.shuffle(baseBeliefs)
        if self.numParticles <= len(baseBeliefs):
            self.particleBeliefs = baseBeliefs[0 : self.numParticles]
        else:
            count = self.numParticles / len(baseBeliefs)
            random.shuffle(baseBeliefs)
            self.particleBeliefs = baseBeliefs * count + baseBeliefs[0 : self.numParticles - count * len(baseBeliefs)]
        ...
        for particle in self.particleBeliefs:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i] is None:
                    ...
                    if emissionModels[i][trueDistance] >= 0: weight *= emissionModels[i][trueDistance]
            jointWeights[particle] += weight

        if jointWeights.totalCount() == 0:
            self.initializeParticles()
        else:
            ## 2. Resample ##
            for num in range(self.numParticles):
                newParticleBeliefs.append(util.sample(jointWeights))
            self.particleBeliefs = newParticleBeliefs
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        """
        newParticles = []
        for oldParticle in self.particleBeliefs:
        ...
        self.particleBeliefs = newParticles
